...
const devPlugins = [
  ...
  new HtmlWebpackPlugin({
    ...
    filename: resolve(`dist/${config.mode}/index.html`),
    inject: true,
...
const prodPlugins = [
  ...
  new InjectManifest({
    ...
    swDest: resolve(`dist/${config.mode}/sw.js`),
    globDirectory: "."
  })
];
...
if (
  config.isProd &&
  checkFilesChange(
    resolve("build/fontmin.txt"),
    resolve("build/scripts/fontmin-hash.txt")
  )
) {
...
const isDirectoryChanged =
  duboConfig.mode === "prod" &&
  (checkFilesChange(
    resolve("src/assets/"),
    resolve("build/scripts/assets-hash.txt")
  ) ||
...
module.exports = basePlugins.concat(config.isProd ? prodPlugins : devPlugins);
