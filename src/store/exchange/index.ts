...
// 先以 $api.product.getGenreList 取得类型资料, 之后整合为 $api.exchange.getGenreList
class ExchangeStore extends BaseStore {
  ...
  getGenreList = async config => {
    const res = await this.$api.product.getGenreList(config);
  ...
  getRuleList = async config => {
    const res = await this.$api.product.getRuleList(config);
