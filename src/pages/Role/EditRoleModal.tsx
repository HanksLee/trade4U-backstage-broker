...
@inject('common')
@observer
export default class EditRoleModal extends BaseReact<IEditRoleModalProps, IEditRoleModalState> {
  state = {
  ...
  handleSubmit = async (evt) => {
    this.props.form.validateFields(async (err, values) => {
      if (!err) {
        ...
        if (!role) {
          this.$api.role.createRole(payload)
            .then(() => {
          ...
          this.$api.role.updateRole(role.id, payload)
            .then(() => {
