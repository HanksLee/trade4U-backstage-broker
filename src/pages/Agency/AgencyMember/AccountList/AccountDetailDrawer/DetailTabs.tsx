...
export default class DetailTabsModal extends BaseReact<IDetailTabsProps, IDetailTabsState> {
  ...
  getAccountDetail = async () => {
    const res = await this.$api.agency.getAccountDetail(this.props.id);
    this.setState({
  ...
  getAccountLoginLog = async (page = 1) => {
    ...
    const res = await this.$api.agency.getAccountLoginLog(this.props.id, { params: { page_size: 10, page, }, });
    this.setState({
