...
@observer
export default class RebateSettingsEditor extends BaseReact<IRebateSettingsEditorProps, IRebateSettingsEditorState> {
  state = {
    mode: 'add',
  ...
  init = async () => {
    ...
    this.setState({
      ...
      if (currentRebate) {
        confirm({
          title: '返佣设置恢复操作',
          content: '检测到您存在未提交的返佣设置记录，请问是否从上次编辑中恢复状态？',
          onOk: () => {
  ...
  renderRebateSettings = () => {
    const { currentRebate: {
      commission_rule,
    }, setCurrentRebate, } = this.props.agency;

    return <FormItem label='返佣规则设置' required {...getFormItemLayout(4, 14)} >
      <Row>
        <Col span={4}></Col>
        <Col span={4}>固定金额</Col>
        <Col span={4}>点值</Col>
      </Row>
      {
        commission_rule.map((item, index) => {
          return <Row>
            ...
            <Col span={4}>
              <InputNumber min={0} value={item.fix_amount} onChange={val => {
                let newCommissionRule = cloneDeep(commission_rule);

                newCommissionRule = newCommissionRule.map((item, subIndex) => {
                  if (index == subIndex) {
                    item.fix_amount = val;
                  }
            ...
            <Col span={4}>
              <InputNumber min={0} value={item.rate} onChange={val => {
                let newCommissionRule = cloneDeep(commission_rule);
  ...
  renderEditor = () => {
    ...
    const { setCurrentRebate, currentShowRebate, currentRebate, } = this.props.agency;
    const {
      agent_rebate_options,
    } = this.state;
    ...
    return (
      <Form className='editor-form'>
        <FormItem label='开启返佣' required {...getFormItemLayout(4, 14)} className='editor-upshelf'>
          {getFieldDecorator('in_use', {
            ...
            <Radio.Group onChange={(evt) => {
              ...
              <Radio style={radioStyle} value={1}>开启</Radio>
              <Radio style={radioStyle} value={0}>关闭</Radio>
            </Radio.Group>
          )}
        </FormItem>
        <FormItem label='自动结算' required {...getFormItemLayout(4, 14)} className='editor-upshelf'>
        ...
        <FormItem label='返佣结算周期' required {...getFormItemLayout(4, 14)} className='editor-upshelf'>
          {getFieldDecorator('cycle', {
            ...
            <Radio.Group onChange={(evt) => {
              ...
              <Radio style={radioStyle} value={'real_time'}>实时</Radio>
              <Radio style={radioStyle} value={'month'}>月</Radio>
            </Radio.Group>
          )}
        </FormItem>
        <FormItem
        ...
        >
          {
            getFieldDecorator('rebate_calculation', {
              initialValue: currentShowRebate && currentShowRebate.rebate_calculation,
            })(
              <Select
                // @ts-ignore
        ...
        {
          !utils.isEmpty(currentShowRebate.commission_rule) && this.renderRebateSettings()
        }
    ...
    this.props.form.validateFields(async (err, values) => {
      if (!err) {
        ...
        if (mode == 'add') {
          ...
          const res = await this.$api.agency.updateRebateSettings(currentAgentId, payload);
