...
interface VerityListState extends IVerityState {
...
export default class VerifyList extends BaseReact<{}, VerityListState> {
  ...
  showEditVerifyModal = (verify?: VerityType) => {
    if (verify) {
      this.setState({
  ...
  render() {
    ...
    return (
      <>
        <div>
          <CommonHeader {...this.props} links={[]} title="审批中心" />
          <Route
            path={`${match.url}/list`}
            render={props => (
