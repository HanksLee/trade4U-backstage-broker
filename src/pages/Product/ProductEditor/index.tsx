...
export default class ProductEditor extends BaseReact<IProductEditorProps, IProductEditorState> {
  ...
  getTransactionModeOptions = async () => {
    const res = await this.$api.product.getTransactionModeOptions({ page: 1, page_size: 200, });
  ...
  getBgColorOptions = async () => {
    const res = await this.$api.product.getBgColorOptions({ page: 1, page_size: 200, });
  ...
  renderEditor = () => {
    ...
    const { setCurrentProduct, currentShowProduct, currentProduct, } = this.props.product;
    const {
      typeOptions,
      marketOptions,
      transactionModeOptions,
    ...
    this.props.form.validateFields(async (err, values) => {
      if (!err) {
        ...
        if (mode == 'add') {
          ...
          const res = await this.$api.product.updateProduct(currentProduct.id, payload);
