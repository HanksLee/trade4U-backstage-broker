        ...
        <div className="order-list">
          {orderDetail ? (
            <>
              <div>
                <div>
                  <span>产品类型</span> {orderDetail.symbol_type_name}
                </div>
                <div>
                  <span>产品名称</span> {orderDetail.symbol_name}
                </div>
                <div>
                  <span>产品杠杆</span> {orderDetail.leverage}
                </div>
                <div>
              ...
              <div>
                <div>
                  <span>止盈</span> {orderDetail.take_profit}
                </div>
                <div>
                  <span>止损</span> {orderDetail.stop_loss}
                </div>
                <div>
                  <span>税金</span> {orderDetail.taxes}
                </div>
                <div>
                  <span>库存费</span> {orderDetail.swaps}
                </div>
                <div>
              ...
              <div>
                ...
                <div>
                  <span>开仓时间</span>{" "}
                  {moment(orderDetail.create_time * 1000).format(
                    "YYYY-MM-DD HH:mm:ss"
                  )}
                </div>
                <div>
                  <span>平仓时间</span>{" "}
                  {moment(orderDetail.close_time * 1000).format(
                    "YYYY-MM-DD HH:mm:ss"
                  )}
                </div>
              </div>
        ...
        <section className="editor-content panel-block">
          ...
          >
            {transactionChoices.map(choice => {
              return (
                <Menu.Item key={choice.field}>{choice.translation}</Menu.Item>
              );
            })}
          </Menu>
          <Table
